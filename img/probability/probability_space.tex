...
level 2/.style ={level distance=3cm, sibling distance=0.8cm},
level 3/.style ={level distance=3cm, sibling distance=0.8cm, dashed},
...
\node (root) {}
	child {node {$x_1$}
   	child {node {$y_1$}
			child {node{$(x_1,y_1)$} edge from parent node[prob] {$P(x_1)P(y_1|x_1)$}}
			edge from parent node[prob] {$P(y_1/x_1)$}
		}
		child {node {$y_2$}
			child {node{$(x_1,y_2)$} edge from parent node[prob] {$P(x_1)P(y_2|x_1)$}}
	...
	child {node {$x_2$}
   	child {node {$y_1$}
			child {node{$(x_2,y_1)$} edge from parent node[prob] {$P(x_2)P(y_1|x_2)$}}
			edge from parent node[prob] {$P(y_1/x_2)$}
		}
		child {node {$y_2$}
			child {node{$(x_2,y_2)$} edge from parent node[prob] {$P(x_2)P(y_2|x_2)$}}
