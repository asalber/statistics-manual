...
segments(188,0,188,93, col="grey", lty=2)
abline(h=69.67, lty=2, col="grey")
arrows(155,69.67,155,93, col=color2, code=3)
text(155,82,"Total variation", pos=4, offset=1)
text(155,79,"$y_j-\\bar y$", pos=4, offset=1)
model = lm(data$weight~data$height)
abline(model,lwd=2)
text(195,110, "Regression line of")
text(195,100, "$Y$ on $X$")
segment(0,coefficients(model)[2]*188+coefficients(model)[1],188,coefficients(model)[2]*188+coefficients(model)[1], col="grey", lty=2)
text(150,coefficients(model)[2]*188+coefficients(model)[1],"$f(x_i)$")
arrows(188,69.67,188,coefficients(model)[2]*188+coefficients(model)[1], col=color2, code=3)
text(188,78,"Explained variation", pos=4, offset=1)
text(188,75,"$f(x_i)-\\bar y$", pos=4, offset=1)
arrows(188,coefficients(model)[2]*188+coefficients(model)[1],188,93, col=color2, code=3)
text(188,89,"Non-explained variation", pos=2, offset=1)
text(188,86,"$e_{ij}=y_j-f(x_i)$", pos=2, offset=1)
dev.off()
